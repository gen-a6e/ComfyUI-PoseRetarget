...
「reference画像の体型・サイズ・配置でdriving画像のポーズ」を取る
OpenPose骨格を作るComfyUIノードです。

referenceのMHR70骨格から3D骨長比を測り、drivingの3Dボーン方向へ適用したあと、
reference側のカメラと焦点距離で2Dへ透視投影します。横向きや手足をカメラへ向けた
...
| `reference_image` | referenceに使った元画像。出力キャンバスの幅・高さを取得するために必要 |
...
| `fit_to_canvas` | off | offはreferenceのサイズを維持。shrink_to_fitははみ出す場合だけ縮小 |
