...
公開出力は、reference体型とdrivingポーズを合成した`pose_keypoint`と、処理内容を
示す`report`の2つです。

drivingのMHR70を直接投影した座標と、SAM内部で投影済みのraw 2D座標はノード内部で
引き続き計算し、診断に使用します。ノード上の比較用ソケットは表示しません。`report`には
左右の手について`raw2d_vs_reprojected`のRMS差・最大差をピクセル単位で表示します。
raw側だけが正しい場合は再投影処理、両方が同じ場合はSAMのMHR70手座標より前段を
調査できます。

旧版の`driving_pose_keypoint`または`sam_raw_driving_pose_keypoint`へ接続していた線は
利用できなくなります。また`report`の出力番号が変わるため、更新後は既存ノードを
追加し直すか、`report`の接続をつなぎ直してください。先頭の`pose_keypoint`は維持します。
...
中心リグまで使えない旧方式へ戻った場合の倍率も直前版どおりです。`torso_scale`はH→69、`head_scale × neck_scale`は肩中央→鼻へ作用します。新方式の比較では必ず`skeleton_mode=full_rig`を確認してください。各倍率の適用先が変わった版から更新する場合は、以前の補正値を一旦1.0を基準に再確認してください。
...
これは3D座標での整列なので、鼻のYや画像上の足元まで一致するとは限りません。`fit_to_canvas=off`ならこの投影配置を維持し、fitを有効にすると2Dでさらに移動・拡縮されます。内部診断用のdriving骨格にはこの整列を適用しません。
OpenPoseの首スロット（BODY18の1番）は、DWPoseと同様に**投影後の左右肩の2D中点**を出力します。公開するマージ結果と、内部で計算するdriving再投影・SAM raw 2Dに共通です。片肩が無効なら首点のconfidenceも0にします。これは出力形式への変換だけで、内部MHR69を肩中点に置き換える処理ではありません。デバッグのSは3D肩中央を投影した点なので、透視投影ではOpenPoseの2D中点と一致しない場合があります。
...
身長とraw 2D診断は補足情報です。内部リグのR126が欠ける・使用不能な場合は該当側の身長を`unavailable`とし、通常のポーズ生成は続行します。SAM内部2Dが欠ける・使用不能な場合もマージ結果は返し、内部のraw診断だけを省略します。再投影座標をraw座標の代わりには使いません。省略理由は`report`の`WARNING`に表示します。MHR70やカメラなど、ポーズ生成に必須の情報が不正な場合は引き続きエラーになります。
